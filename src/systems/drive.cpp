...
void turnToHighMogo(QAngle target){
    auto su = Settled(200, 1, 0.2);
    pidGains turnGains {2.4, 1, 20, 25, 15};
...
void turnToWithMogo(QAngle target){
    auto su = Settled(200, 1, 0.2);
    pidGains turnGains {1.5, 0.5, 18, 50, 15};
    PID turnPID(turnGains);
    double error, mappedError;
    do {
        error = target.convert(degree) - inertialSensor.get_heading();
...
void driverToggleBrake(){
    if (toggleHoldMode.changedToPressed()){
        isHold = (isHold==1)?0:1;
        ToggleBrakeModeTask.notify();
    }
}
